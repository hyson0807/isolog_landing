...
function featureOne(frame) {
  ...
  return svg(`
    ${phoneMockup(325, 34, 310, 652, (sx, sy, sw, sh) => {
      ...
      return `
        ...
        <g opacity="${settle}">
          ${rect(sx + 32, sy + 566, sw - 64, 46, 18, "#ecfdf5", "#bbf7d0", 1.2)}
          <circle cx="${sx + 58}" cy="${sy + 589}" r="13" fill="${green}"/>
          ${checkPath(sx + 58, sy + 589, "#ffffff", 0.75, settle)}
          ${centerText(sx + sw / 2 + 16, sy + 590, "Logged at 9:41 PM", 15, 850, "#166534", `opacity="${settle}"`)}
        </g>
      `;
    }, 1)}
  `);
...
function featureTwo(frame) {
  ...
  return svg(`
    ${phoneMockup(330, 38, 300, 644, (sx, sy, sw, sh) => {
      const noteY = lerp(sy - 88, sy + 64, slide);
      return `
        ${phoneStatus(sx, sy, sw)}
        ...
        <g opacity="${clamp((t - 0.04) / 0.18) * clamp((0.95 - t) / 0.15)}">
          ${rect(sx + 16, noteY, sw - 32, 92, 28, "#ffffff", "#d8eefb", 1.4, 'filter="url(#cardShadow)"')}
          <circle cx="${sx + 52}" cy="${noteY + 46}" r="${20 + pulse * 4}" fill="#fff3e8"/>
          <path d="M${sx + 52} ${noteY + 30}c-10 0-16 7-16 18v8h32v-8c0-11-6-18-16-18Z" stroke="${brand}" stroke-width="3.6" stroke-linecap="round" fill="none"/>
          <path d="M${sx + 44} ${noteY + 61}h16" stroke="${brand}" stroke-width="3.6" stroke-linecap="round"/>
          ${text(sx + 84, noteY + 40, "Dose reminder", 14, 800, muted)}
          ${text(sx + 84, noteY + 68, "10 mg at 10:00 PM", 20, 850, ink)}
        </g>

        ${card(sx + 22, sy + 150, sw - 44, 128, 30, "#ffffff", "#d8eefb")}
        ${text(sx + 48, sy + 190, "Next dose", 15, 800, blue)}
        ${text(sx + 48, sy + 238, "10:00 PM", 43, 850, ink)}
        ${rect(sx + sw - 94, sy + 194, 48, 48, 16, "#eff6ff", "#bfdbfe", 1.2)}
        <path d="M${sx + sw - 70} ${sy + 207}v20l14 8" stroke="${blue}" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/>

        ${card(sx + 22, sy + 302, sw - 44, 126, 30)}
        ${text(sx + 48, sy + 342, "Schedule", 15, 800, muted)}
        ${text(sx + 48, sy + 382, "Every other day", 28, 850, ink)}
        ${rect(sx + 48, sy + 398, 132, 12, 6, "#e8edf3")}
        ${rect(sx + 48, sy + 398, 86, 12, 6, brand)}
        <circle cx="${sx + 204}" cy="${sy + 404}" r="6" fill="${teal}"/>
        <circle cx="${sx + 226}" cy="${sy + 404}" r="6" fill="#cbd5e1"/>

        ${card(sx + 22, sy + 454, sw - 44, 96, 28, "#f8fffd", "#b7eee5")}
        ${text(sx + 48, sy + 494, "Notifications", 15, 800, teal)}
        ${text(sx + 48, sy + 526, enabled > 0.45 ? "Enabled" : "Ready", 25, 850, enabled > 0.45 ? green : ink)}
        <rect x="${sx + sw - 106}" y="${sy + 494}" width="62" height="34" rx="17" fill="${enabled > 0.45 ? brand : "#cbd5e1"}"/>
        <circle cx="${lerp(sx + sw - 88, sx + sw - 62, enabled)}" cy="${sy + 511}" r="13" fill="#ffffff"/>
      `;
    }, 1)}
  `);
}
  ...
  return svg(`
    ${phoneMockup(325, 34, 310, 652, (sx, sy, sw, sh) => {
      ...
      return `
        ...
        ${card(sx + 22, sy + 548, sw - 44, 72, 24, "#ffffff", "#dbeafe")}
        ${text(sx + 48, sy + 586, "Goal", 15, 800, muted)}
        ${text(sx + 92, sy + 587, "25,000 mg", 23, 850, blue)}
        <defs>
          <clipPath id="sparkClip">
            <rect x="${sx + 190}" y="${sy + 568}" width="${76 * p}" height="34"/>
          </clipPath>
        </defs>
        <path d="M${sx + 190} ${sy + 594}C${sx + 208} ${sy + 570} ${sx + 225} ${sy + 606} ${sx + 244} ${sy + 584}S${sx + 286} ${sy + 576} ${sx + 266} ${sy + 600}" stroke="${blue}" stroke-width="3.4" stroke-linecap="round" fill="none" clip-path="url(#sparkClip)"/>
      `;
    }, 2)}
  `);
}
